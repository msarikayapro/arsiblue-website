...
@if ($position === 'head')
    ...
    {{-- GA4 (only if GTM not active — GTM içinden GA4 yönetimi tercih) --}}
    @if (! setting('gtm_container_id') && setting('ga4_measurement_id'))
        <script async src="https://www.googletagmanager.com/gtag/js?id={{ setting('ga4_measurement_id') }}"></script>
        <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
        gtag('js', new Date());
        gtag('config', '{{ setting('ga4_measurement_id') }}');
        </script>
    @endif

    {{-- Google Ads --}}
    @if (setting('google_ads_conversion_id') && ! setting('gtm_container_id'))
        <script async src="https://www.googletagmanager.com/gtag/js?id={{ setting('google_ads_conversion_id') }}"></script>
        <script>
        window.dataLayer = window.dataLayer || [];
        function gtag(){dataLayer.push(arguments);}
